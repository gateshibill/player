...
class LiveRcmdPage extends StatefulWidget {
  LiveRcmdPage({Key key, @required this.vod,this.pc});
 PlayerController pc;
  ChannelModel vod;

  @override
  _LiveRcmdPageState createState() => _LiveRcmdPageState(vod: this.vod,pc:this.pc);
}

class _LiveRcmdPageState extends State<LiveRcmdPage> {
  _LiveRcmdPageState({Key key, @required this.vod,this.pc});
  PlayerController pc;
  ChannelModel vod;
  Map infoData = {
    "personInfo": {
  ...
  Widget moreVideo() {
    if (infoData['moreVideo'].length != 0) {
      List<Widget> list = [];
      sportsChannelList[0].forEach((e) {
        list.add(
          InkWell(
            onTap: () {
              pc.play(e.playUrl,e.name);
              this.vod=e;
            },
            child: Container(
